...
function getFullNames(runners) {
  let arr = [];
  runners.forEach(function(curr) {
    let last = curr.last_name;
    let first = curr.first_name;
    arr.push(`${last}, ${first}`);
  });
  return arr;
}
...
function firstNamesAllCaps(runners) {
  let people = runners.map(function(curr){
    return curr.first_name.toUpperCase(); 
  });
  return people;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
    let arr = runners.filter(function(item){
...
function tallyUpDonations(runners) {
  let result = runners.reduce(function(accumulator, currentValue) {
